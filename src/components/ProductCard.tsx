  ...
  return (
    ...
    >
      <Link to={`/producto/${product.slug}`} className="relative aspect-square overflow-hidden">
        <img src={product.image} alt={product.name} className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 ${product.stock === 0 ? "opacity-40 grayscale" : ""}`} loading="lazy" />
        {product.stock === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="bg-destructive text-destructive-foreground font-display text-lg px-4 py-2 rounded-lg tracking-wider uppercase">Agotado</span>
          </div>
        )}
