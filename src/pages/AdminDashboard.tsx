...
import { Loader2, LogOut, Plus, Pencil, Trash2, Package, AlertTriangle, Tag, ShoppingBag, Ticket } from "lucide-react";
import AdminProductForm from "@/components/admin/AdminProductForm";
import AdminSalesLog from "@/components/admin/AdminSalesLog";
import AdminCoupons from "@/components/admin/AdminCoupons";

type Tab = "products" | "sales" | "coupons";
        ...
        {tab === "products" && (
          <>
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <Input placeholder="Buscar producto..." value={search} onChange={e => setSearch(e.target.value)} className="bg-card border-border flex-1" />
                    ...
                    <tbody>
                      {filtered.map((p: any) => (
                        <tr key={p.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                          ...
                          <td className="p-3 text-right">
                            <span className={p.stock === 0 ? "text-destructive" : p.stock <= 10 ? "text-yellow-500" : ""}>{p.stock}</span>
                          </td>
